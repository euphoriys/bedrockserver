...
pd login ubuntu -- bash -c "yes | apt update && yes | apt upgrade && yes | apt install nano curl wget gpg"
...
pd login ubuntu -- bash -c "cd ~ && wget https://ryanfortner.github.io/box64-debs/box64.list -O /etc/apt/sources.list.d/box64.list && wget -qO- https://ryanfortner.github.io/box64-debs/KEY.gpg | gpg --dearmor -o /etc/apt/trusted.gpg.d/box64-debs-archive-keyring.gpg && yes | apt update && yes | apt install box64-android"
