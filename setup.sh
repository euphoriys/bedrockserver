...
pd login ubuntu -- yes | apt update && pd login ubuntu -- yes | apt upgrade && pd login ubuntu -- yes | apt install nano git curl wget gpg
...
pd login ubuntu -- cd ~ && pd login ubuntu -- wget https://raw.githubusercontent.com/invinc1ble7/bedrockserver/main/server && pd login ubuntu -- mv server ../usr/bin/server && pd login ubuntu -- chmod +x ../usr/bin/server
